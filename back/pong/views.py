...
# Using is_valid(), authenticate() functions with "is not None"
# Python/Django functions and tools
def sign_in(request):
	...
	if request.method == 'POST':
		...
		if registerform.is_valid():
			...
			if UserProfile.objects.filter(email=email).exists():
				...
				return redirect('home')
			new_user = UserProfile.objects.create_user(username=username, password=password, email=email)
			# return redirect('verify-view')
...
def save_image(image_url):
    # Adding verify=False to bypass SSL verification - Note: this is insecure
...
# never_cache is a decorator that tells the browser not to cache the response
# to this view, every time the view is called, the check will happen.
