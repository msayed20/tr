...
function attachEvent() {
    ...
    document.getElementById('playSingle').addEventListener('click', function() {
        ...
        else if (activeLanguage.textContent.trim().toLowerCase() === 'arabic') {
            document.getElementById('playDiv').innerHTML = `
            <div class="mb-2 text-center">
                <p class="mb-2"><span id="backArrow" style="cursor: pointer;">
                    <i class="bi bi-arrow-left text-light me-1"></i>
                </span>العب مباراة 1 ضد 1:</p>
                <button class="btn btn-dark btn-sm shadow-sm text-light col-6 mb-2" data-bs-toggle="modal" data-bs-target="#localMatchModal">محلي</button>
                <button class="btn btn-primary btn-sm shadow-sm text-light col-6" data-bs-toggle="modal" data-bs-target="#onlineMatchModal">عبر الإنترنت</button>
            </div>
            `;
        }
    ...
    document.getElementById('playTournament').addEventListener('click', function() {
        ...
        if (activeLanguage.textContent.trim().toLowerCase() === 'english') {
            ...
            <div class="mb-2 text-center">
                ...
                <button class="btn btn-dark btn-sm shadow-sm text-light col-6 mb-2" data-bs-toggle="modal" data-bs-target="#localTournament">محلي</button>
                <button class="btn btn-primary btn-sm shadow-sm text-light col-6" data-bs-toggle="modal" data-bs-target="#onlineTournament">عبر الإنترنت</button>
            </div>
            `;
        }
        document.getElementById('backArrow').addEventListener('click', function() {
