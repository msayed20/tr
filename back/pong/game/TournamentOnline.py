...
class TournamentOnline():
    ...
    def add_player_to_game(self, player):
        ...
        for game in self.games:
            if (game.player1 == "" or game.player2 == ""):
                ...
                # connect here with the BD to pair players 2 by 2
                break
        if player.game == None:
    ...
    def started(self):
        for player in self.players:
            if player.player_status in ["Disqualified", "Qualified"]:       #browse players, ignore if qualified/disqualified
                continue
            if player.player_status == "Waiting" and player.game and (player.game.player1 == "" or player.game.player2 == ""): #qualifies the player if he is alone
                player.player_status = "Qualified"
            if player.game and player.game.has_finished and player.game.player1 == player.name:     # If the game is over, look at the score and determine a winner.
                if player.game.p1_score <= player.game.p2_score:
                    player.player_status = "Disqualified"
                else:
                    player.player_status = "Qualified"
            elif player.game and player.game.has_finished and player.game.player2 == player.name:   # same for player 2
                if player.game.p2_score <= player.game.p1_score:
                    ...
                    player.player_status = "Qualified"
                    player.game = None
        if all(player.player_status in ["Qualified", "Disqualified"] for player in self.players):    #runs the players, if they are all qualified/disqualified, enters the conditions
            if (self.qualif == 1):       #final played, gives a winner for the tournament
                self.status = "Ending"
                for player in self.players:
                    player.game = None
